...
class SimSession:
	...
	def write_csv_status(self,x_step,y_step,z_step):
		if self.csvWriter!=None:
			...
			csv_row.append(self.displacement_diff) #2
			csv_row.append(self.displaced_weight) #3
			csv_row.append(z_step) #4

			csv_row.append(self.cg_empty.location.z) #5

			csv_row.append(degrees(self.cg_empty.rotation_euler.y))  #6 pitch
			csv_row.append(self.y_arm) #7
			csv_row.append(y_step) #8

			csv_row.append(degrees(self.cg_empty.rotation_euler.x)) #9 roll
			csv_row.append(self.x_arm) #10
			csv_row.append(x_step) #11

			self.csvWriter.writerow(csv_row)


	def report_status(self,z_step):

		statusText=("s:%d f: %d z: %0.3f dd:%0.3f zs:%0.4f rY:%0.3f Ya:%f rX:%0.3f Xa:%0.4f"%(
	...
	def run_simulation(self,limitsteps):
		...
		while continue_simulation:
			...
			if self.simulation_step==0:
				...
				if (self.force_roll_max==0 and z_step>0) or (self.force_roll_max>0 and x_step>0):
					self.bake_keyframes()
	...
	def check_simulation_finished(self,limitsteps):
		...
		if self.cg_empty.location.z>self.water_volume.dimensions.z:
			print("Aborting... Hull Z: %f > water height: %f"%(self.cg_empty.location.z,self.water_volume.dimensions.z))
			return True

		# Abort if runaway...
		if self.simulation_step>limitsteps:
			print("Aborting - suspected runaway simulation")
			return True
		...
		if self.force_roll_max==0:
			...
			if ( (self.simulate_pitch and abs(self.x_arm)<self.arm_solve_threshold) or self.simulate_pitch==False) and \
				...
				print("Finished - all parameters stabilized")
				return True
		else:
			# we are doing a forced rollover - 
			# Abort forced rollover test when we reach the max rollover angle
			if self.rotationX>=self.force_roll_max:
					print("Finished - rolltest reached max roll target")
					return True
